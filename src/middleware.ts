...
export const config = {
  ...
  // /api/lembretes (cron de lembretes — Story 8.2) e /api/retencao (cron de
  // retenção — Story 9.1), ambos com autenticação própria via Bearer secret,
  // não por sessão, rotas internas do Next, assets e favicon.
  matcher: [
    "/((?!login|agendar|api/lembretes|api/retencao|_next/static|_next/image|favicon.ico).*)",
  ],
};
